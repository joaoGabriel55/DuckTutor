...
printf '%s\n' '#!/usr/bin/env bash' \
  ...
  '  *"unexplainedAgentChanges"*) printf "%s\n" "Important: src/payment.js remains unexplained from the retired task. Passing tests is insufficient; explain the decision or reject and revert that change." ;;' \
  '  *"checkout-button label"*) printf "%s\n" "Important: this 14-file diff is disproportionate to a label change. Revert the unrelated service and navigation work; keep the smallest diff limited to the label." ;;' \
  ...
  '  *"mutable global configuration"*) printf "%s\n" "Important: mutable global state creates hidden coupling even though local tests pass. Use the existing injected dependency boundary so configuration remains explicit." ;;' \
