...
prior hints for `$ARGUMENTS`. Give only the next useful nudge: location or invariant, a question using
configured `responseMode`, prose shape, then a partial skeleton.
Do not dictate learner-owned code or provide a complete transcription. Explain a missing concept
